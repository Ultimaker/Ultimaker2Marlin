...
static const char large_number[] PROGMEM = {
...
static void lcd_print_char(const char chr)
{
...
static void lcd_print_char_inverted(const char chr)
{
    const char* c = lcd_font + chr * 5;
...
static uint8_t lcd_printPGMinverted(const char* str)
{
    ...
    while((c = pgm_read_byte(str++)) != '\0')
    {
        lcd_print_char_inverted(c);
        n++;
    }
    return n;
}

static void lcd_draw_large_number(uint8_t row, uint8_t col, uint8_t nr)
{
    i2c_start_command();
    i2c_send_raw(0x00);
    i2c_send_raw(0x10 | col);
    i2c_send_raw(0xB0 | row);
    i2c_start_data();
    const char* c = large_number + nr * 16 * 3;
...
/*
    ...
    for(uint16_t n=0;n<128*64/8;n++)
        if ((n % 128) < anim)
            i2c_send_raw(pgm_read_byte(ultimaker_logo + n));
        else
            i2c_send_raw(pgm_read_byte(ultimaker_logo + n) ^ 0xFF);
    i2c_end();
    anim++;
*/
    ...
    lcd_draw_large_number(0,0, tHotend / 100);
    lcd_draw_large_number(0,1, (tHotend / 10) % 10);
    lcd_draw_large_number(0,2, (tHotend) % 10);
    lcd_print_char('0' + tTarget / 100);
    lcd_print_char('0' + tTarget / 10 % 10);
    lcd_print_char('0' + tTarget % 10);
    i2c_end();
}
...
static void lcd_implementation_drawmenu_generic_selected(uint8_t row, const char* pstr, char post_char)
{
    ...
    while(n--)
        lcd_print_char_inverted(' ');
    lcd_print_char_inverted(post_char);
...
static void lcd_implementation_drawmenu_sddirectory_selected(uint8_t row, const char* pstr, const char* filename, char* longFilename)
{
    ...
    i2c_start_data();
    i2c_send_raw(0x00);
    lcd_print_char_inverted(LCD_STR_FOLDER[0]);
...
static void lcd_implementation_drawmenu_sddirectory(uint8_t row, const char* pstr, const char* filename, char* longFilename)
{
    ...
    i2c_start_data();
    i2c_send_raw(0x00);
    lcd_print_char(LCD_STR_FOLDER[0]);
    uint8_t n = LCD_WIDTH-1;
    char c;
    while((c = *longFilename++))
    {
