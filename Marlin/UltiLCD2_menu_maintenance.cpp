...
static void lcd_menu_maintenance_change_material_settings();
...
static char* lcd_advanced_item(uint8_t nr)
{
    if (nr == 0)
        ...
        strcpy_P(card.longFilename, PSTR("Change mat. settings"));
    else if (nr == 3)
#if EXTRUDERS < 2
...
static void lcd_menu_maintenance_advanced()
{
    ...
    if (lcd_lib_button_pressed)
    {
        if (IS_SELECTED_SCROLL(0))
            ...
            lcd_change_to_menu(lcd_menu_maintenance_change_material_settings, 0);
...
static char* lcd_change_material_items(uint8_t nr)
{
    if (nr == 0)
        strcpy_P(card.longFilename, PSTR("< RETURN"));
    else if (nr == 1)
        ...
        strcpy_P(card.longFilename, PSTR("Wait time"));
    else
        strcpy_P(card.longFilename, PSTR("???"));
    return card.longFilename;
}

static void lcd_change_material_details(uint8_t nr)
{
    char buffer[16];
    if (nr == 0)
        return;
    ...
    {
        int_to_string(change_material_temperature, buffer, PSTR("C"));
    }
    else if(nr == 2)
    {
        int_to_string(change_material_preheat_wait_time, buffer, PSTR("Sec"));
    }
    lcd_lib_draw_string(5, 53, buffer);
}
...
static void lcd_menu_maintenance_change_material_settings()
{
    lcd_scroll_menu(PSTR("CHANGE MAT. SETTINGS"), 3, lcd_change_material_items, lcd_change_material_details);
    if (lcd_lib_button_pressed)
    {
        if (IS_SELECTED_SCROLL(0))
        {
            lcd_change_to_menu(lcd_menu_maintenance_advanced, SCROLL_MENU_ITEM_POS(2));
        }
        else if (IS_SELECTED_SCROLL(1))
        {
            LCD_EDIT_SETTING(change_material_temperature, "Change mat. temp.", "C", 0, HEATER_0_MAXTEMP - 15);
        }
        else if (IS_SELECTED_SCROLL(2))
        {
            LCD_EDIT_SETTING(change_material_preheat_wait_time, "Change mat. delay.", "Sec", 0, 120);
        }
    }
}
