...
//                float mm_e = current_block->steps_e / axis_steps_per_unit[E_AXIS];
    ...
    lcd_lib_draw_string_leftP(17, PSTR("Z"));
    ...
    LCDMenu::drawMenuString(LCD_CHAR_MARGIN_LEFT+12
                          ...
                          , 0);
    // flow
    //if (e_smoothed_speed[active_extruder] > 0.01)
//    {
//        float_to_string1(e_smoothed_speed[active_extruder], buffer, PSTR(UNIT_FLOW));
//        lcd_lib_draw_gfx(LCD_CHAR_MARGIN_LEFT-1, 24, flowGfx);
////        lcd_lib_draw_string(LCD_CHAR_MARGIN_LEFT+12, 24, buffer);
//        LCDMenu::drawMenuString(LCD_CHAR_MARGIN_LEFT+12
//                      , 24
//                      , 10*LCD_CHAR_SPACING
//                      , LCD_CHAR_HEIGHT
...
//    }
    ...
    // speed
    // if (current_nominal_speed > 0.01)
    {
        lcd_lib_draw_gfx(LCD_CHAR_MARGIN_LEFT, 30, speedGfx);
        int_to_string(current_nominal_speed+0.5, buffer, PSTR(UNIT_SPEED));
        // lcd_lib_draw_string(LCD_CHAR_MARGIN_LEFT+12, 33, buffer);
        LCDMenu::drawMenuString(LCD_CHAR_MARGIN_LEFT+12
                              , 30
                              , 7*LCD_CHAR_SPACING
                              , LCD_CHAR_HEIGHT
    ...
    // start animation
#if TEMP_SENSOR_BED != 0
    lcd_lib_draw_gfx(LCD_GFX_WIDTH-LCD_CHAR_MARGIN_RIGHT-4*LCD_CHAR_SPACING-11, 30, fan1Gfx);
#else
    ...
    default:
        ...
        {
            lcd_lib_draw_string_leftP(BOTTOM_MENU_YPOS, (movesplanned() < 1) ? PSTR("Paused...") : PSTR("Pausing..."));
        }
        else
        {
            ...
            else if (movesplanned() > 1)
            {
                lcd_lib_draw_string_leftP(BOTTOM_MENU_YPOS, PSTR("Printing..."));
            }
            else
            {
                lcd_lib_draw_string_leftP(BOTTOM_MENU_YPOS, PSTR(WELCOME_MSG));
            }

        }

        break;
    case PRINT_STATE_DWELL:
        lcd_lib_draw_string_leftP(BOTTOM_MENU_YPOS, PSTR(MSG_DWELL));
        break;
    case PRINT_STATE_WAIT_USER:
        ...
        {
            lcd_lib_draw_string_leftP(BOTTOM_MENU_YPOS, PSTR("Pausing..."));
        }
        break;
    }
