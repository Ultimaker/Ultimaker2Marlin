...
void lcd_menu_printing_tg()
{
    ...
    //                float mm_e = current_block->steps_e / axis_steps_per_unit[E_AXIS];
        ...
        if (card.pause)
        {
            ...
            lcd_lib_draw_string_rightP(15, (movesplanned() < 1) ? PSTR("Paused...") : PSTR("Pausing..."));
        }
        else
        {
            // time left
    ...
    case PRINT_STATE_WAIT_USER:
        ...
        if (movesplanned() < 1)
        {
            lcd_lib_draw_string_rightP(15, PSTR("Paused..."));
            lcd_lib_draw_string_leftP(BOTTOM_MENU_YPOS, PSTR("Click to continue..."));
        }
        else
        {
            lcd_lib_draw_string_rightP(15, PSTR("Pausing..."));
        }
        break;
